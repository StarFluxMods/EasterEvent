using KitchenData;
using KitchenLib.Customs;
using KitchenLib.References;
using KitchenLib.Utils;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Reflection;
using System.Text;
using System.Threading.Tasks;
using UnityEngine;

namespace EasterEvent.Customs
{
	public class Easter_Eggs_Dish : CustomDish
	{
		//CustomGameDataObject
		public override string UniqueNameID => "Easter_Eggs_Dish";
		public override int BaseGameDataObjectID => DishReferences.BurgerBase;

		//CustomUnlock
		public override Unlock.RewardLevel ExpReward => Unlock.RewardLevel.Medium;
		public override bool IsUnlockable => true;
		public override UnlockGroup UnlockGroup => UnlockGroup.Dish;
		public override CardType CardType => CardType.Default;
		public override int MinimumFranchiseTier => 0;
		public override bool IsSpecificFranchiseTier => false;
		public override DishCustomerChange CustomerMultiplier => DishCustomerChange.None;
		public override float SelectionBias => 0;
		public override List<Unlock> HardcodedRequirements => new List<Unlock> { };
		public override List<Unlock> HardcodedBlockers => new List<Unlock> { };

		public override bool IsAvailableAsLobbyOption => true;
		public override GameObject IconPrefab => Main.bundle.LoadAsset<GameObject>("DishIcon");

		public override List<string> StartingNameSet => new List<string>
		{
			"Fresh outta clucks",
			"Chick me out!",
			"You're one funny chick",
			"You're a good egg",
			"What an egg-citing day",
			"Are you an Easter eggs-pert?",
			"Bunny, I’m home!",
			"Happy Eggs-ter!",
			"Hey there, hop stuff"
		};

		//CustomDish
		public override string AchievementName => "";
		public override DishType Type => DishType.Main;

		public override HashSet<Item> MinimumIngredients => new HashSet<Item>()
		{
			RefVars.Blue_Dye,
			RefVars.White_Dye,
			RefVars.Red_Dye,
			RefVars.Yellow_Dye,
			RefVars.Chocolate,
			RefVars.Chocolate_Mold_Empty
		};


		public override HashSet<Process> RequiredProcesses => new HashSet<Process>
		{
			RefVars.Cook,
			RefVars.Freeze
		};

		public override List<Dish.MenuItem> ResultingMenuItems => new List<Dish.MenuItem>
		{
			new Dish.MenuItem
			{
				Item = RefVars.Red_White,
				Phase = MenuPhase.Main,
				Weight = 1,
				DynamicMenuType = DynamicMenuType.Static,
				DynamicMenuIngredient = null
			},
			new Dish.MenuItem
			{
				Item = RefVars.Red_Blue,
				Phase = MenuPhase.Main,
				Weight = 1,
				DynamicMenuType = DynamicMenuType.Static,
				DynamicMenuIngredient = null
			},
			new Dish.MenuItem
			{
				Item = RefVars.Blue_White,
				Phase = MenuPhase.Main,
				Weight = 1,
				DynamicMenuType = DynamicMenuType.Static,
				DynamicMenuIngredient = null
			}
		};
		public override HashSet<Dish.IngredientUnlock> IngredientsUnlocks => new HashSet<Dish.IngredientUnlock>
		{
			new Dish.IngredientUnlock
			{
				MenuItem = RefVars.Red_White,
				Ingredient = RefVars.Red_Dye
			},
			new Dish.IngredientUnlock
			{
				MenuItem = RefVars.Red_White,
				Ingredient = RefVars.White_Dye
			},
			new Dish.IngredientUnlock
			{
				MenuItem = RefVars.Red_White,
				Ingredient = RefVars.Chocolate
			},
			new Dish.IngredientUnlock
			{
				MenuItem = RefVars.Red_Blue,
				Ingredient = RefVars.Red_Dye
			},
			new Dish.IngredientUnlock
			{
				MenuItem = RefVars.Red_Blue,
				Ingredient = RefVars.Blue_Dye
			},
			new Dish.IngredientUnlock
			{
				MenuItem = RefVars.Red_Blue,
				Ingredient = RefVars.Chocolate
			},
			new Dish.IngredientUnlock
			{
				MenuItem = RefVars.Blue_White,
				Ingredient = RefVars.White_Dye
			},
			new Dish.IngredientUnlock
			{
				MenuItem = RefVars.Blue_White,
				Ingredient = RefVars.Blue_Dye
			},
			new Dish.IngredientUnlock
			{
				MenuItem = RefVars.Blue_White,
				Ingredient = RefVars.Chocolate
			},
			new Dish.IngredientUnlock
			{
				MenuItem = RefVars.Red_White,
				Ingredient = RefVars.Egg_Basket
			},
			new Dish.IngredientUnlock
			{
				MenuItem = RefVars.Red_Blue,
				Ingredient = RefVars.Egg_Basket
			},
			new Dish.IngredientUnlock
			{
				MenuItem = RefVars.Blue_White,
				Ingredient = RefVars.Egg_Basket
			}
		};
		public override Dictionary<Locale, string> Recipe => new Dictionary<Locale, string>
		{
			{ Locale.English, "Melt Chocolate, Mold and Freeze. Add paint ready to serve!" }
		};

		public override void OnRegister(GameDataObject gameDataObject)
		{
			Dish dish = (Dish)gameDataObject;
			LocalisationObject<UnlockInfo> info = new LocalisationObject<UnlockInfo>();
			FieldInfo dictionary = ReflectionUtils.GetField<LocalisationObject<UnlockInfo>>("Dictionary");
			Dictionary<Locale, UnlockInfo> dict = new Dictionary<Locale, UnlockInfo>();
			dict.Add(Locale.English, new UnlockInfo
			{
				Name = "Easter Eggs",
				Description = "Adds Easter Eggs as a Main",
				FlavourText = "Those Easter eggs are really to dye for"
			});
			dictionary.SetValue(info, dict);
			dish.Info = info;
		}
	}
}
